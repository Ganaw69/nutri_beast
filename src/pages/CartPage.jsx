    ...
    <div className="bg-[#131313] min-h-screen text-[#e5e2e1] font-heading py-12 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-black text-4xl sm:text-5xl text-white uppercase tracking-tight">
            YOUR CART
          </h1>
        </div>

        {cart.length === 0 ? (
          <div className="bg-[#181818] border border-white/10 rounded-lg p-12 text-center space-y-4">
            ...
            <p className="text-xs text-gray-400 font-body">Découvrez notre gamme de nutrition sportive haut de gamme.</p>
            <button
              onClick={() => navigateTo("shop")}
              className="bg-[#d90429] text-white font-black text-xs uppercase px-8 py-4 rounded-xs inline-flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-7 space-y-4">
              {cart.map((item) => (
                <div
                  key={item.id}
                  className="bg-[#181818] border border-white/10 p-6 rounded-lg relative font-heading flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6"
                >
                  <button
                  ...
                  <div className="bg-white p-3 rounded-md w-28 h-28 flex items-center justify-center shrink-0 relative shadow-md">
                    <img
                      src={item.image}
                      alt={item.name}
                  ...
                  <div className="flex-1 space-y-3 text-center sm:text-left">
                    ...
                    <div className="inline-flex items-center bg-[#0e0e0e] border border-white/10 rounded-xs px-3 py-1.5 font-bold text-xs text-white gap-4">
                      <button
            ...
            <div className="lg:col-span-5">
              <div className="bg-[#181818] border border-white/10 p-6 sm:p-8 rounded-lg space-y-6 font-heading">
                ...
                <form onSubmit={handleApplyPromo} className="bg-[#0e0e0e] border border-white/10 rounded-xs p-3 flex justify-between items-center text-xs">
                  <input
                    type="text"
                    placeholder="PROMO CODE"
                    value={promoCode}
                    onChange={(e) => setPromoCode(e.target.value)}
                    className="bg-transparent text-white placeholder-gray-500 font-bold text-xs uppercase focus:outline-none flex-1 pr-2"
                  />
                  <button
                    type="submit"
                    className="text-[#d90429] font-black uppercase hover:text-white transition-colors tracking-wider"
                  >
                    APPLY
                  </button>
                </form>
                ...
                <div className="text-[10px] font-bold text-gray-400 flex items-center justify-center gap-1.5 uppercase tracking-wider">
